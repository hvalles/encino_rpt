...
2. stdlib imports (`io`, `sys`, `json`, `csv`, `datetime`, `decimal`, `dataclasses`, `typing`, `ast`, `math`, `pathlib`, `enum`)
3. relative intra-package imports (`from ._specs import FieldSpec` in `encino_rpt/report.py:7`; `from ..models import Image, Link` in `encino_rpt/renderers/csv.py:9`; `from .._serialize import to_jsonable` in `encino_rpt/renderers/json.py:7`)
4. optional-dependency imports are **always function-local** (never top-level)

**Path Aliases:**
- No path aliases configured. Only `pythonpath = ["."]` in `pyproject.toml:43`. No `import`-rewriting.

**Lazy imports (cycle-breaking + optional deps):**
...
**Size:** Functions are small and single-purpose; the engine is decomposed into many private `_`-prefixed helpers in `encino_rpt/aggregation.py` (each phase of `build()` is its own function).
...
**Return Values:** Fluent builder methods return `self` (or `Section`) for chaining; validation raises early.
...
**Barrel Files:** `encino_rpt/__init__.py` and `encino_rpt/renderers/__init__.py` are the only barrel files; both use explicit `__all__`.
